...
class CommandHelp extends CommandCLI {
    ...
    exec(args: string[], client: CLI, commandUsage: string) {
        ...
        if (command) {
            ...
            console.log(bold(yellow(`---- `)) + bold(red(`Help: ${command.id}`)) + bold(yellow(` ----`)))
            console.log(bold(yellow(`Descrição: `)) + bold(white(`${command.options.description.content}`)))
            console.log(bold(yellow(`Usável: `)) + bold(white(`${command.options.description.usage.replace('[command]', command.id)}`)))
            console.log(bold(yellow(`Aliases: `)) + `[${bold(white(`${aliases.join(', ')}`))}]`)
            return;
        }

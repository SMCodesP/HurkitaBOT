...
class Mp3Search extends Command {
  ...
  async exec(
    message: Message,
    { searchQuery, page = null }: { page: string | null; searchQuery: string }
  ) {
    const tableConvert = {
      1: '1️⃣',
      2: '2⃣',
    ...
    responseData.items.forEach((item, index) => {
      ...
      embedSearch
        .addField(`🔢 Item »`, tableConvert[index + 1], false)
        .addField(
          `🖊️ Título »`,
          `\`\`\`yaml\n${item.snippet.title}\`\`\``,
          false
    ...
    const collectorReaction = messageSearch.createReactionCollector(filter, {
      time: 60000 * 5,
    })
    ...
    collectorReaction.on('collect', async (reaction) => {
      if (functionsReactions[reaction.emoji.name]) {
        ...
        const itemSelect =
          Number(this.getKeyByValue(tableConvert, reaction.emoji.name)) - 1
        await messageSearch.delete()

...
class Mp3Volume extends Command {
  ...
  constructor() {
    super('mp3volume', {
      ...
      description: {
        content:
          'Com esse comando você pode aumentar o volume de uma música mp3!',
  ...
  async exec(message: Message, { newVolume }: { newVolume: number }) {
    if (!validatePermission(message.author, 'mp3volume'))
      return message.util.reply(
        'você não tem permissão para aumentar o volume do mp3.'
      )
    if (!message.member.voice.channel)
      return message.util.reply('você não está em um canal de voz!')
    if (!newVolume || !Number(newVolume))
      return message.util.reply(
        `sintaxe incorreta, utilize o comando dessa forma \`${getPrefix(
          message.guild.id
        )}${this.id} [1-10]\`.`
      )
    ...
    if (newVolume < 1 || newVolume > 10)
      return message.util.reply(
        `você não pode aumentar o volume maior de 10 ou menor que 1.`
      )

    if (newVolume > 5) {
      const messageConfirm = await message.util.reply(
        'você realmente deseja aumentar o volume maior que 5?\nUm volume maior que 5 podem prejudicar sua audição, reaga com ✅ para confirmar.'
      )
      ...
      const collectorReaction = messageConfirm.createReactionCollector(filter, {
        time: 60000 * 5,
      })

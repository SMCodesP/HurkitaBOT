  ...
  constructor() {
    super('animeimage', {
      ...
      description: {
        ...
        examples: ['[command] waifu', '[command] list'],
      },
      args: [
        {
          id: 'type',
  ...
  async exec(message: Message, { type }: { type: string }) {
    ...
    try {
      ...
      const embed = new MessageEmbed()
        .setColor('RANDOM')
        .setDescription(
          `Clique [aqui](${image.data.url}) para baixar a imagem.`
        )
      ...
      message.util.reply(
        'Houve um erro ao buscar uma imagem, tente novamente mais tarde.'
      )
    }
  }
}

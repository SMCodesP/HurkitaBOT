...
type typeOfTypes = "manga" | "novels" | "oneshots" | "doujin" | "manhwa" | "manhua";
...
class TopMangasCommand extends Command {
  ...
  async exec(message: Message, { type, page }: { type: string | undefined, page: number }) {
    ...
    if (type === "list") {
      ...
      Object.keys(types).forEach((typeManga) => {
        embedTypes.addField(`> ${typeManga}`, `\`\`\`yaml\n${prefix}${this.id} ${typeManga}\`\`\``, true)
      })
    ...
    try {
      if (!Object.values(types).includes(typeMangaSelected as typeOfTypes) && typeMangaSelected !== undefined) {
        message.util.reply(`o tipo de top de mangá digitado é inválido, utilize:\n${Object.keys(types).map(typeManga => `\`${typeManga}\``).join('\n')}`)
        return;
      }

      const result = typeMangaSelected === undefined ? await JikanTS.Top.items("manga", 1) : await JikanTS.Top.items("manga", 1, typeMangaSelected as typeOfTypes);
      ...
      (result.top.slice(10 * (page - 2), 10 * (page - 1)).length > 0) && await messageTopList.react("⬅️");
      (result.top.slice(10 * (page), 10 * (page + 1)).length > 0) && await messageTopList.react("➡️");

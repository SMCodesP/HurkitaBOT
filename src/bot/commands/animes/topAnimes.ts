...
class TopAnimesCommand extends Command {
	...
	async exec(message: Message, { type, page }: { type: string | undefined, page: number }) {
		...
		try {
			...
			if (!Object.values(types).includes(typeAnimeSelected as typeOfTypes) && typeAnimeSelected !== undefined) {
				message.util.reply(`o tipo de top de anime digitado é inválido, utilize:\n${Object.keys(types).map(typeAnime => `\`${typeAnime}\``).join('\n')}`)
				return;
			}

			const result = typeAnimeSelected === undefined ? await JikanTS.Top.items("anime", 1) : await JikanTS.Top.items("anime", 1, typeAnimeSelected as typeOfTypes);

...
class NSFWWallpaperAnimeCommand extends Command {
  ...
  constructor() {
    super('nwallpaperanime', {
      ...
      description: {
        content:
          'Com esse comando você pode gerar um wallpaper NSFW de acordo com sua pesquisa.',
  ...
  async exec(message: Message, { query }: { query: string }) {
    try {
      ...
      if (message.channel.type !== 'text')
        throw new Error(
          `Você não pode executar um comando NSFW nesse canal, para um wallpaper SFW use \`${prefix}aw ${query}\``
        )
      if (!message.channel.nsfw)
        throw new Error(
          `O canal não é um NSFW então você não pode executar esse comando aqui, para um wallpaper SFW use \`${prefix}aw ${query}\`.`
        )
      ...
      const sendWallPaper = async (page: number) => {
        const messageWallpaper = await message.util.reply(
          new MessageEmbed()
            ...
            .setDescription(
              `Clique [aqui](${thumbs[page]}) para baixar o wallpaper.`
            )

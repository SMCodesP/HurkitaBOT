	...
	async exec(message: Message, { serverIp }: { serverIp?: string }) {
		...
		try {
			...
			const embed = new MessageEmbed()
				...
				.setDescription(`Status do servidor: \`${serverIp}\`\n\n\n**Motd »**\n\`\`\`yaml\n${response.motd.clean.toString().trim()}\`\`\``)
				.addField("> **Status »**", `\`\`\`yaml\nonline\`\`\``, true)
				.addField("> **Players »**", `\`\`\`yaml\n${response.players.online}/${response.players.max}\`\`\``, true)
				.addField("> **Versão »**", `\`\`\`diff\n- ${response.version}\`\`\`\n\u200B`, false)
				.setTimestamp()
				.setFooter(`Copyright © 2020 - ${this.client.user.username}`, this.client.user.displayAvatarURL())
			...
			const embed = new MessageEmbed()
				...
				.setColor("RANDOM")
				.setDescription(`Status do servidor: \`${serverIp}\``)
				.addField("\u200B", `<:error:799624539226767412> ${error.message || "Houve um erro ao buscar as informações."}\n\u200B`)
				.setTimestamp()
				.setFooter(`Copyright © 2020 - ${this.client.user.username}`, this.client.user.displayAvatarURL())

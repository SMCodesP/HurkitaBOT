...
class CreateBotRole extends Command {
        ...
        },
        {
          ...
          prompt: {
            start: (message: Message) =>
              `${message.author}, **Digite as permissões para o cargo.**`,
  ...
  exec(
    message: Message,
    { rolename, permissions }: { rolename: string; permissions: string }
  ) {
    ...
    if (!rolename)
      return message.util.reply(
        `por favor você tem que digitar o nome do cargo no comando, dessa forma \`${
          db.get(`${message.guild.id}.prefix`) || process.env.PREFIX
        }createbotrole [nome do cargo]\`.`
      )

    if (db.get(`roles.${rolename}`))
      return message.util.reply(
        'você não criar um cargo com o mesmo nome de outro.'
      )

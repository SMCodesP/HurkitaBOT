...
class AddBotRole extends Command {
  constructor() {
    super('addbotrole', {
      ...
      description: {
        ...
        metadata:
          'Add role; setar permissão; set permission; set role; dar cargo; role; cargo;',
        usage: '[command] [role] [@user/userID]',
        examples: ['[command] owner @SMCodes#4207'],
      },
      args: [
        {
          id: 'rolename',
  ...
  exec(
    message: Message,
    {
      rolename,
      memberMention,
    }: { rolename: string; memberMention: GuildMember }
  ) {
    if (!validatePermission(message.author, 'addbotrole'))
      return message.util.reply(
        'você não tem permissão para setar um cargo a um usuário.'
      )
    if (message.author.id === memberMention.user.id || !rolename)
      return message.util.reply(
        `Sintaxe incorreta, digite dessa forma \`${
          db.get(`${message.guild.id}.prefix`) || process.env.PREFIX
        }addbotrole [nome do cargo] [@user]\``
      )

    const role: RoleBot = db.get(`roles.${rolename}`)
    if (!role)
      return message.util.reply(
        `o cargo digitado não existe dentro do meu sistema, use \`${
          db.get(`${message.guild.id}.prefix`) || process.env.PREFIX
        }listarcargos\``
      )
    ...
    message.util.reply(
      `você setou o cargo \`${role.name}\` para o usuário ${memberMention}.`
    )
  }
}

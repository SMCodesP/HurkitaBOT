...
class ListColorCommand extends Command {
  ...
  constructor() {
    super('listcolors', {
      ...
      description: {
        content:
          'Com esse comando você pode listar todas as cores disponíveis.',
  ...
  async exec(message: Message, { page }: { page: number }) {
    ...
    const {
      data: { color_role },
    } = database.find((item) => item.ID === message.guild.id)
    const collection_roles: Collection<string, Role> = new Collection()

    Object.values(color_role).forEach((color: { id: string }) => {
    ...
    const messageReply = await message.util
      .reply(`**Página ${page} | Lista de cores disponíveis »**\n\n${
      !roles[page - 1]
    ...
    if (roles[page]) await messageReply.react('➡️')
    if (roles[page - 2]) await messageReply.react('⬅️')

    if (roles[page - 2] || roles[page]) {
      const filter = (_: MessageReaction, user: User) =>
        user.id === message.author.id
      const collectorReaction = messageReply.createReactionCollector(filter, {
        time: 60000 * 5,
        max: 1,
      })

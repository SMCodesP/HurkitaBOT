...
class AvatarCommand extends Command {
            ...
            args: [
                {
                    ...
                    type: async (message: Message, member: GuildMember | string) => {
                        ...
                        if (typeof member === "string") {
                            try {
                                return (await message.guild.members.fetch()).get(member) || message.member
                            } catch (error) {
    ...
    async exec(message: Message, { memberMention }: { memberMention: GuildMember }) {

        try {
            const urlAvatar = memberMention.user.displayAvatarURL({ size: 4096, dynamic: true })

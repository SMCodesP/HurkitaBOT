...
class HelpCommand extends Command {
  ...
  constructor() {
    super('help', {
      ...
      description: {
        ...
        metadata:
          'Comando de ajudar; ajuda; help; ?; outros comandos; me ajuda; help-me; ajude-me;',
  ...
  async helpCommand(message: Message, name: string) {
    ...
    const embedHelpOfCommand = new MessageEmbed()
      ...
      .setDescription(
        '**Atenção »** Argumentos de comandos que tenham `[]` são obrigatórios, com `{}` são argumentos opicionais.\n \u200B'
      )
      .addField('Nome »', `\`\`\`diff\n- ${commandSelected.id}\`\`\``, true)
      .addField(
        'Alternativas »',
      ...
      .addField(
        ...
        }${commandSelected.description.usage.replace(
          '[command]',
          commandSelected.id
        )}\`\`\``,
        false
      ...
      .addField(
        'Exemplos »',
        `\`\`\`yaml\n${commandSelected.description.examples
          .join('\n')
          .replace(
            /\[command\]/g,
            `${db.get(`${message.guild.id}.prefix`) || process.env.PREFIX}` +
  ...
  async helpCategory(message: Message, name: string) {
    const categorySelected = this.handler.categories.get(
      this.handler.categories.findKey((key) => {
        const strignsOfKey = key.id.split('|')
        if (!strignsOfKey[1]) return false
    ...
    if (!categorySelected) throw new Error('Nenhuma categoria encontrada.')
    ...
    const emoji =
      this.client.emojis.cache.find((emoji) => {
        return (
          emoji.id === categoryFullName.split(' ')[0] ||
          emoji.name === categoryFullName.split(' ')[0]
        )
    ...
    const embedHelpOfCategory = new MessageEmbed()
      .setColor('RANDOM')
      .setTitle(
        `Lista de comandos da categoria: __${emoji} ${categoryFullName
          .split(' ')[1]
          .trim()}__`
      )
      .setDescription(
        `❗ **Prefix »** \`${
          db.get(`${message.guild.id}.prefix`) || process.env.PREFIX
        }\`\n📄 **Comandos disponíveis nessa categoria »** \`${
    ...
    categorySelected.forEach((command) => {
      embedHelpOfCategory.addField(
        `\u200B`,
        `> **\`${
          db.get(`${message.guild.id}.prefix`) || process.env.PREFIX
        }${command.description.usage.replace(
          '[command]',
          command.aliases[0]
  ...
  async exec(message: Message, { select }: { select: string }) {
    ...
    if (select && typeof select === 'string' && !this.isNumeric(select)) {
      this.helpCategory(message, select).catch((_) => {
        this.helpCommand(message, select).catch((_) => {
          message.reply(
            `Não foi possível localizar um comando nem uma categoria com esse nome, utilize \`${
              db.get(`${message.guild.id}.prefix`) || process.env.PREFIX
            }pesquisar [Alguma palavra chave para encontrar o nome do comando]\``
          )
        })
      })
    ...
    const organize = () => {
      return new Map(
        [...this.handler.categories.entries()].sort(
    ...
    const embed = new MessageEmbed()
      .setTitle(
        `Ajuda da Hurkita${typeof page === 'number' ? ` | Page ${page}` : '.'}`
      )
      .setColor('RANDOM')
      .setDescription(
        `❗ **Prefix »** \`${
    ...
    const categories: string[] =
      typeof page === 'number' ? pages[page - 1] : [...organize().keys()]
    ...
    categories.forEach(async (categoryKey) => {
      ...
      const emoji =
        this.client.emojis.cache.find((emoji) => {
          return (
            emoji.id === name.split(' ')[0] || emoji.name === name.split(' ')[0]
          )
        }) || name.split(' ')[0]

      embed.addField(
        `> ${emoji} ${name.split(' ')[1]} \`[${
          countCommandsInCategory.size
        } comandos]\``,
        `\`${db.get(`${message.guild.id}.prefix`) || process.env.PREFIX}${
          this.id
        } ${nameFormatted}\`\n \u200B`
    ...
    for (let indexHelp = 0; indexHelp < categories.length; indexHelp++) {
      ...
      const collectorReaction = messageHelpEmbed.createReactionCollector(
        filter,
        { time: 60000 * 5, max: 1 }
      )

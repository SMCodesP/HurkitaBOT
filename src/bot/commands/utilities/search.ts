...
class SearchCommand extends Command {
    ...
    constructor() {
        super("search", {
            ...
            description: {
                content: "Com esse comando você pode pesquisar um outro comando com apenas características dele.",
    ...
    async exec(message: Message, { query }: { query: string }) {
        const embedOfSearching = new MessageEmbed()
                .setColor("RANDOM")
                .setTitle("🔎 Buscando...")
                .setDescription("⚠️ **Atenção »** Essa ferramenta é experimental, provavelmente não funcionará com todos os comandos.\n \u200B")
                .setTimestamp()
        ...
        try {
            const response = await this.client.sonicChannelSearch.query("commands_test", "default", query, {
                lang: "por",
                limit: 3
            })

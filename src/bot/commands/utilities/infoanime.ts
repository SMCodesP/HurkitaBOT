    ...
    async exec(message: Message, { animeTitle }: { animeTitle: string }) {
        ...
        try {
            ...
            const embedAnime = new MessageEmbed()
                ...
                .addField("Gêneros »", `\`\`\`yaml\n[${response.genres.map((genre) => genre.name).join(', ')}]\`\`\``, false)
                .addField("Transmitido »", `\`\`\`yaml\n${response.type}\`\`\``, true)
                .addField("Studios »", `\`\`\`yaml\n[${response.studios.map((studio) => studio.name).join(', ')}]\`\`\``, true)
                .addField("Membros »", `\`\`\`yaml\n${response.members}\`\`\``, false)
                .addField("Popularidade »", `\`\`\`yaml\n${response.popularity}\`\`\``, true)

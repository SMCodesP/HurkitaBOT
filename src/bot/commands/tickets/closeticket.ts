...
class CloseTicket extends Command {
  constructor() {
    super('closeticket', {
      ...
      description: {
        ...
        metadata:
          'Comando para deletar tickets; delete; fechar; close; fechar ticket; close ticket; delete ticket;',
      ...
      args: [
        {
          id: 'member',
          default: (message: Message): GuildMember =>
  ...
  async exec(message: Message, { member }: { member: GuildMember }) {
    const userTickets: Array<Ticket> = db.get(
      `tickets.${message.guild.id}.${member.user.id}`
    )

    if (userTickets) {
      if (userTickets.filter((ticket) => !ticket.closed).length <= 0)
        return message.util.reply(
          'você não pode deletar um ticket caso não tenha um aberto.'
        )
    }
    ...
    if (
      ...
      return message.util.reply(
        'você não tem permissão para deletar um ticket de outro membro.'
      )
    ...
    message.util.reply(
      `você fechou o ticket \`#${
        ticket.id
      }\` com sucesso!\nCaso teve algum problema com o ticket ele pode ser reconstruido usando, \`${db.get(
        `${message.guild.id}.prefix`
      )}reconstituirticket ${
        ticket.id
      }\` assim poderá obter as logs do ticket em um novo canal de ticket.`
    )
  }
}

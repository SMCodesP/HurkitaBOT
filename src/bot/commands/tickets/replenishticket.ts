  ...
  async exec(message: Message, { ticket_id }: { ticket_id: Number }) {
    if (!ticket_id)
      return message.util.reply(
        'Por favor execute o comando novamente com o id do ticket, ele é obrigatório.'
      )

    const guildTickets: Object = db.get(`tickets.${message.guild.id}`)

    if (guildTickets) {
      const usersTicketsParsed: Array<Array<Ticket>> = Object.values(
        guildTickets
      )
      ...
      const ticket = concatAllTickets.find(
        (ticket: Ticket) => ticket.id === ticket_id && ticket.closed
      )

      if (!ticket)
        return message.util.reply(
          'Não existe ou o ticket com esse id não está fechado.'
        )
      ...
      if (
        ...
        creator.user.id !== message.author.id
      )
        return message.util.reply(
          'você não tem permissão para reconstituir um ticket de outro membro.'
        )

      if (!creator)
        return message.util.reply(
          'O criador do ticket não existe nesse servidor.'
        )
      ...
      const embedReconstitutingTicket = new MessageEmbed()
        ...
        .setDescription(
          '📄 Pode demorar um tempo para terminar a reconstituição.'
        )
        .addField(
          '🔧 Criador do ticket »',
          `\`\`\`yaml\n${creator.user.tag}\`\`\``,
      ...
      const messageReconstituing = await message.util.reply(
        embedReconstitutingTicket
      )

      let categoryChannel = message.guild.channels.cache.find(
        (channelCache) =>
          channelCache.name === 'Reconstituições' &&
          channelCache.type === 'category'
      )
      ...
      if (categoryChannel) {
        reconstitutedChannel = await message.guild.channels.create(
          `ticket-${ticket.id}`,
          {
            type: 'text',
      ...
      const createWebhooksPromise = ticket.content.map(
        async (ticketMessage) => {
          ...
          if (!webhooks[member.user.id]) {
            webhooks[member.user.id] = await reconstitutedChannel.createWebhook(

...
class ClearCommand extends Command {
  constructor() {
    super('clear', {
      ...
      description: {
        ...
        metadata:
          'Comando para apagar mensagens; clear messages; clear; limpeza;',
        usage: '[command] [1-100]',
        examples: ['[command] 50', '[command] 1'],
      },
      args: [
        {
          id: 'messagesOfDelete',
  ...
  async exec(message: Message, args: { messagesOfDelete: number }) {
    ...
    if (message.channel.type == 'dm' || message.channel.type == 'news')
      return message
        .reply('Não posso apagar mensagens nesse canal.')
        .then((messageNotPossibleDelete) =>
          messageNotPossibleDelete.delete({ timeout: 2500 })
        )

    if (args.messagesOfDelete < 1 && args.messagesOfDelete > 100)
      return message
        .reply(
          'Você não pode apagar mais de 100 mensagens, nem menos de 1 mensagem!'
        )
        .then((messageNotDeleteOfLimitMessages) =>
          messageNotDeleteOfLimitMessages.delete({ timeout: 2500 })
        )

...
class LogCommand extends Command {
  constructor() {
    super('log', {
      ...
      description: {
        content:
          'Com esse comando um administrador pode setar um log em um canal.',
        metadata: 'Comando para log; setar; set; definir;',
        usage: '[command] [Tipo de log]',
        examples: ['[command] banimentos', '[command] expulcoes'],
      },
      args: [
        {
          id: 'type_log',
  ...
  async exec(message: Message, { type_log }: { type_log: string }) {
    if (!type_log) return message.util.reply('Você não definiu um tipo de log.')
    ...
    message.util.reply(
      `Você definiu o canal ${message.channel} como um canal de log \`${type_log}\``
    )
  }
}

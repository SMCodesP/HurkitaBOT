...
class BotClient extends AkairoClient implements BotClientTypes {
  ...
  constructor() {
    ...
    this.registerCommandHandler({
      ...
      argumentDefaults: {
        prompt: {
          modifyStart: (_: Message, text: string): string =>
            `${text}\n\nDigite \`cancelar\` para cancelar a sessão do comando.`,
          modifyRetry: (_: Message, text: string): string =>
            `${text}\n\nDigite \`cancelar\` para cancelar a sessão do comando.`,
          timeout:
            'Você demorou muito para digitar, a sessão do comando foi cancelada',
          ended:
            'Você antingiu o máximo de tentativas na sessão com comando, então ela foi cancelada.',

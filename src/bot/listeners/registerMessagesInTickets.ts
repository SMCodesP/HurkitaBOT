...
class RegisterMessageInTicketsListener extends Listener {
  ...
  exec(message: Message) {
    ...
    if (guildTickets) {
      ...
      const ticket: Ticket = concatAllTickets.find(
        (ticket: Ticket) => ticket.channel_id === message.channel.id
      )
      if (ticket) {
        // @ts-ignore
        ...
        ticketsUserOfAddMessage = concatAllTickets
          .filter(
            (ticketReplacedPreview: Ticket) =>
              ticket.creator === ticketReplacedPreview.creator
          )
          .map((ticketReplaced: Ticket) =>
            ticketReplaced.id === ticket.id ? ticket : ticketReplaced
          )

        db.set(
          `tickets.${message.guild.id}.${ticket.creator}`,
          ticketsUserOfAddMessage
        )
      }
    }
  }
